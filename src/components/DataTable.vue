<template>
  <div ref="dataTable" class="vue3-easy-data-table" :class="[tableClassName]">
    ...
    >
          ...
          <tr>
              ...
              :class="[
                {
                  ...
                  none: header.sortable && header.sortType === Sort.NONE,
                  desc: header.sortable && header.sortType === Sort.DESC,
                  asc: header.sortable && header.sortType === Sort.ASC,
            ...
            >
              ...
              <span v-else class="header" :class="`direction-${headerTextDirection}`">
                ...
                <i
                  v-if="header.sortable && header.sortType !== Sort.NONE"
                  :key="header.sortType ? header.sortType : Sort.NONE"
                  class="sortType-icon"
                  :class="{ desc: header.sortType === Sort.DESC }"
                ></i>
      ...
      <div v-if="loading" class="vue3-easy-data-table__loading">
    ...
    <div v-if="!hideFooter" class="vue3-easy-data-table__footer">
      <div v-if="!hideRowsPerPage" class="pagination__rows-per-page">
        {{ rowsPerPageMessage }}
...
import { useSlots, computed, toRefs, ref, watch, provide, onMounted } from 'vue';
...
import type { Header, Item, HeaderForRender } from '@/types/table';

import { generateColumnContent } from '@/utils/getValue';
import { type PropsWithDefault } from '@/types/propsWithDefault';
...
const isMultipleSelectable = computed((): boolean => !!itemsSelected.value);
const isServerSideMode = computed((): boolean => !!serverOptions.value);
...
const { totalItems, selectItemsComputed, totalItemsLength, toggleSelectAll, toggleSelectItem } =
  useTotalItems(
    clientSortOptions,
    filterOptions,
    isServerSideMode,
...
const { currentPageFirstIndex, currentPageLastIndex, multipleSelectStatus, pageItems } =
  usePageItems(
    currentPaginationNumber,
    isMultipleSelectable,
    isServerSideMode,
...
const { expandingItemIndexList, updateExpandingItemIndexList, clearExpandingItemIndexList } =
