...
const paginationItemsForRender = computed((): PaginationItem[] => {
  ...
  if (props.maxPaginationNumber <= TOTAL_VISIBLE) {
    // x,x,x,x
    ...
    [1, 2, props.maxPaginationNumber, props.maxPaginationNumber - 1].includes(
      props.currentPaginationNumber
    )
  ) {
    // x,x,x,...,x,x,x
    for (let i = 1; i <= TOTAL_VISIBLE; i += 1) {
      if (i <= 3) {
        paginationItems.push({
          type: 'button',
          page: i,
        ...
        const page = props.maxPaginationNumber - (TOTAL_VISIBLE - i);
        paginationItems.push({
          type: 'button',
          page,
    ...
    [props.maxPaginationNumber - 2, props.maxPaginationNumber - 3].includes(
      props.currentPaginationNumber
    )
  ) {
    // x,...,x,x,x,x,x
