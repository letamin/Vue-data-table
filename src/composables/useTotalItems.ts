import type { ClientSortOptions, FilterOption } from '@/types/actions';
import type { EmitsEventName } from '@/types/events';
import type { Item } from '@/types/table';
import { getItemValue } from '@/utils/getValue';
import { type Ref, computed, type ComputedRef, watch } from 'vue';

export default function useTotalItems(
  clientSortOptions: Ref<ClientSortOptions | null>,
  filterOptions: Ref<FilterOption[] | undefined>,
  isServerSideMode: ComputedRef<boolean>,
  items: Ref<Item[]>,
  itemsSelected: Ref<Item[] | undefined>,
  ...
  const generateSearchingTarget = (item: Item): string => {
    if (typeof searchField.value === 'string' && searchField.value !== '') {
      return getItemValue(searchField.value, item) as string;
    }
    ...
    if (filterOptions.value) {
      filterOptions.value.forEach((option: FilterOption) => {
        itemsFiltered = itemsFiltered.filter((item) => {
          ...
          const itemValue = getItemValue(field as string, item) as number;
          switch (comparison) {
            case '=':
            ...
            case 'in':
              return (criteria as number[]).includes(itemValue);
            default:
              return itemValue === criteria;
          }
        });
  ...
  function recursionMultiSort(
    ...
    const sorted = (
      index === 0 ? itemsToSort : recursionMultiSort(sortByArr, sortDescArr, itemsToSort, index - 1)
    ).sort((a: Item, b: Item) => {
      let isAllSame = true;
