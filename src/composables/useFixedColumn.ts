import type { HeaderForRender } from '@/types/table';
...
export default function useFixedColumn(headersForRender: Ref<HeaderForRender[]>) {
  const fixedHeaders = computed((): HeaderForRender[] =>
    headersForRender.value.filter((header) => header.fixed)
  );
  ...
  const fixedColumnsInfos = computed((): FixedColumnsInfo[] => {
    ...
    return fixedHeaders.value.map(
      (header: HeaderForRender, index: number): FixedColumnsInfo => ({
        value: header.value,
        ...
        distance:
          index === 0
            ? 0
            : fixedHeadersWidthArr.reduce(
                (previous: number, current: number, i: number): number => {
